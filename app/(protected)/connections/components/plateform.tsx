...
const SocialsPlateform: React.FC<SocialsPlateformProps> = ({ icon, socialPlateform, description, btnText, btnColor, onButtonClick, grid }) => {
  return (
    <div className={`flex bg-[#ECF8CB] md:bg-inherit p-6 md:p-0 rounded-xl flex-col gap-4 md:flex-row ${grid ? 'flex-col items-start' : 'md:items-center md:justify-between mb-8'}`}>
      <div className={`flex flex-col md:flex-row ${grid ? 'flex-col items-start gap-2' : 'md:items-center gap-4'}`}>
